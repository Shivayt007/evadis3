...
from utils import get_size, is_subscribed, get_poster, search_gagala, temp, get_settings, save_group_settings, get_shortlink, send_all, check_verification, get_token,get_short
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        try:
            ...
            [
                [
                    InlineKeyboardButton('📥 File Download Link 📥 - KPSliNks', url=g)
                ],
                [
                    InlineKeyboardButton('📁 File to Direct Link 📁', url='https://t.me/How_To_Download_OR_Online_Watch/10')
                ],
                [
                    InlineKeyboardButton('🤔 How to Download 🤔', url='https://t.me/tgtamillinks/2')
                ]]))
...
async def auto_filter(client, msg, spoll=False):
    ...
    btn.insert(0, [
        InlineKeyboardButton("⚡ How To Download ⚡", url=f"https://t.me/tgtamillinks/2")
    ])
    btn.insert(0, [
        InlineKeyboardButton("⚡ Cʜᴇᴄᴋ Bᴏᴛ PM ⚡", url=f"https://t.me/{temp.U_NAME}")
    ])
    ...
    if imdb and imdb.get('poster'):
        ...
        fuk = await message.reply_message(text=cap, reply_markup=InlineKeyboardMarkup(btn))
        try:
            if settings['auto_delete']:
                await asyncio.sleep(600)
                await fuk.delete()
